...
var newscrunch_customize_control_tabs = function ( $ ) {
	'use strict';
		...
		function () {
				...
				$( '.newscrunch-tabs-control' ).each(
					function () {
						...
						var shownCtrls = $( this ).find( '.newscrunch-customizer-tab > input:checked' ).data( 'controls' );
						showControls( customizerSection, shownCtrls );
					}
				);

				$( '.newscrunch-customizer-tab > label' ).on(
					'click', function () {
						var customizerSection = $( this ).closest( '.accordion-section' );
...
newscrunch_customize_control_tabs( jQuery );
